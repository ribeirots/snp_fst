...
#        else:
#            filename = '/Volumes/4_TB_RAID_Set_1/DPGP2plus/wrap1kb/Chr3R/AllCounts_'+pops[p1]+'_NoInv_Chr'+arm+'.txt'
	filename = '/Volumes/4_TB_RAID_Set_1/DPGP2plus/wrap1kb/inv_free_counts/AllCounts_KF_NoInv_Chr3R.txt'
        ...
        with open(filename) as counts_file:
            ...
            for line in counts_file:
                ...
                if sample_size_full >= cutoff_dict[p1][arm]:
                    ...
                    if sum(p1_pi_line) > cutoff_dict[p1][arm]:
                        pi_site = down_sample(p1_pi_line,cutoff_dict[p1][arm])
                    else:
                        pi_site = p1_pi_line[:]
        ...
        pi_p1_output = 'pi\tChr'+arm+'\t'+p1+'\t'+str(pi_p1_final)+'\n'
        ...
        for p2 in range(0,len(pops)):
            dxy_p1p2 = []
            line_count_p2 = -1
            if pops[p2] == 'FRht':
                filename2 = '../FR_heteroz/AllCounts_'+pops[p2]+'_NoInv_Chr'+arm+'_diploids.txt'
            else:
                filename2 = '/Volumes/4_TB_RAID_Set_1/DPGP2plus/wrap1kb/Chr3R/AllCounts_'+pops[p2]+'_NoInv_Chr'+arm+'.txt'

            outfile_snp_fst = open('empirical_snp_fst_'+p1+'_'+pops[p2]+'_NoInv_Chr'+arm+'downsampled.txt','w')
            ...
            with open(filename2) as counts_file2:
                ...
                for line2 in counts_file2:
                    ...
                    if sample_size2_full >= cutoff_dict[pops[p2]][arm] and len(sites_p1[line_count_p2]) == 4:
                        ...
                        if sum(l1) == cutoff_dict[p1][arm]:
                            l1 = l1
                        elif sum(l1) > cutoff_dict[p1][arm]:
                            l1 = down_sample(l1,cutoff_dict[p1][arm])
                        else:
                            min_max_skip = 1
            ...
            output_pi_dxy.write('dxy\tChr'+arm+'\t'+p1+'\t'+pops[p2]+'\t'+str(dxy12)+'\n')
