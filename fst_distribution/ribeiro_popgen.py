...
def sim_recomb_list(model,recrate_dir, recrate_file,chrm, p1_code, p2_code, replicates=n_sim):
    snp_dict = {}
    win_dict = {}
...
def pvalue_recrates(win_full,empirical_summ, sim, emp, p1, p2, outputfilename): # win_full: path+file to window_chr basic summary.  empirical_summ: to empirical fst per window. sim = model (eg. NA_3L). emp = empirical data (eg. 2R).
    recs = []
    ...
    for i in range(0,len(recombination_rate_dir)):
        dicts_rec = sim_recomb_list(sim, recombination_rate_dir[i], recombination_rate_fil[i], emp, p1, p2)
